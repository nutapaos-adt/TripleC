...
| `users` | **Firebase Auth uid** for real accounts (seed data also has legacy arbitrary ids like `user_ward01` — display-only, never referenced by new writes) | `name`, `role`, `email` |
...
Read from the Firebase Auth **custom claim** `role` (set via `setup-users.js`, never from a Firestore
field a client could edit) — `ward_staff`, `home_visit_team`, `admin`. Full capability matrix: see
[ACL.md](ACL.md).
