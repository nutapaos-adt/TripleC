...
## สเปกระบบ + ผู้ช่วย AI + ชุดทดสอบ (อัปเดตสัปดาห์ที่ 9)

- **สเปกรวม**: ดู [spec.md](spec.md) — รวม `SCOPE.md` + โค้ดจริง 3 สัปดาห์ที่ผ่านมาเป็นไฟล์เดียว (หน้าจอ,
  โครงสร้างข้อมูล, บทบาท, สิ่งที่ไม่ทำ) ใช้เป็นใบสั่งงานหลักของทุกงานหลังจากนี้
- **ผู้ช่วย AI 3 ตัว** ใน [`.claude/agents/`](../.claude/agents/) แบ่งตามงาน คนละโมเดล — `ui-screens`
  (haiku, ดูแลหน้าจอ), `data-firestore` (opus, ดูแล schema/rules — จุดเสี่ยงสูงสุด), `ai-buttons`
  (sonnet, ดูแลปุ่ม AI) และ `tester` (sonnet, เขียน/รันชุดทดสอบ)
- **ชุดทดสอบ**: [`tests/`](tests/) เขียนด้วย Playwright รันจริงกับเว็บ live — รันเองด้วย
  `npm test` (ต้อง `npx playwright install chromium` ครั้งแรก) ผลล่าสุดอยู่ที่
  [test-results.md](test-results.md) (ผ่านครบ 5/5 รวมเทสต์ความปลอดภัย 2 ตัว)
- **สิ่งที่ยังไม่เสร็จ**: ดู [`../BACKLOG.md`](../BACKLOG.md)
